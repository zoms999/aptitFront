      ...
      <div className="relative bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/40 p-10 hover:shadow-3xl transition-all duration-500">
        ...
        {questions.map((question, questionIndex) => (
          <div key={question.qu_code} className={`transition-opacity duration-300 ${questionIndex > 0 ? 'border-t border-gray-200/80 pt-10 mt-10' : ''}`}>
            ...
            {/* ✅ [수정] 문항 번호와 제목 표시 방식을 PureTextQuestionTemplate과 동일하게 변경 */}
            <div className="flex items-baseline gap-4 md:gap-5 mb-8">
              <span className="text-2xl md:text-3xl font-bold text-amber-600 flex-shrink-0">
                {question.qu_order}.
              </span>
              <p className="text-xl md:text-2xl text-slate-800 leading-relaxed font-semibold">
                {/* 논리 템플릿에서는 qu_title이 주로 문제의 도입부/전체 지시문 역할을 하므로 qu_title을 사용합니다. */}
                {question.qu_title}
              </p>
            </div>

            {/* ✅ [수정] 헤더 변경에 따라 컨텐츠를 들여쓰기 위한 래퍼 추가 */}
            <div className="pl-8 md:pl-12">
              {/* 논리 지문 */}
              {question.qu_passage && question.qu_passage.trim() !== '' && (
                <div className="p-6 bg-gradient-to-br from-amber-50 to-yellow-50 rounded-2xl border-2 border-amber-200/50 shadow-lg">
