...
import { TemplateProps, TimerState, Question } from './types';
import { 
  CircularProgress, 
  formatTime, 
  getTimerProgress, 
...
export default function PureTextQuestionTemplate({ testData, selectedAnswers, onSelectChoice }: TemplateProps) {
  ...
  const stableQuestions = useMemo(() => {
    ...
    console.log('[PureTextQuestion] 문항 로드:', questions.map(q => ({
      qu_code: q.qu_code,
        ...
        {stableQuestions.map((question, questionIndex) => {
          ...
          return (
            <div key={question.qu_code} className={`transition-opacity duration-300 ${questionIndex > 0 ? 'border-t border-gray-100 pt-10 mt-10' : ''}`}>
              ...
              {/* 선택지 */}
              {showChoices ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4 ml-20">
                  {question.choices.map((choice) => (
                    <div key={choice.an_val} className="relative group/choice">
                      ...
                      <button 
                        ...
                        className={`relative w-full h-full p-3 text-center rounded-xl font-semibold transition-all hover:scale-105 hover:-translate-y-1 ${
                          selectedAnswers[question.qu_code] === choice.an_val 
                            ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-xl scale-105 -translate-y-1' 
                            : 'bg-white/90 border border-gray-200/60 text-gray-700 hover:bg-white'
                        }`}
                      >
                        <div className="flex flex-row items-center justify-center h-full">
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs flex-shrink-0 transition ${
                            selectedAnswers[question.qu_code] === choice.an_val 
