...
export async function GET() {
  try {
    ...
    console.log('일반회원 대시보드 - 사용자 정보:', JSON.stringify(session.user, null, 2));
    ...
    if (userAny.ac_id) {
      try {
        ...
        console.log('일반회원 대시보드 - ac_id로 계정 조회 결과:', JSON.stringify(accountResult, null, 2));
        ...
        console.error('일반회원 대시보드 - 계정 조회 오류:', queryError);
        throw queryError;
      }
    } else if (userAny.id) {
      // id가 ac_gid인 경우
      try {
        ...
        console.log('일반회원 대시보드 - id를 ac_gid로 사용:', ac_gid);
        
        // ac_gid 유효성 검증 및 일반 회원 확인
        ...
        console.error('일반회원 대시보드 - ac_gid 검증 오류:', queryError);
        throw queryError;
      }
      ...
      try {
        ...
        console.log('일반회원 대시보드 - 이메일로 계정 조회:', userEmail);
        ...
        console.log('일반회원 대시보드 - 이메일로 개인정보 조회 결과:', JSON.stringify(personResult, null, 2));
        ...
        console.log('일반회원 대시보드 - pe_seq로 계정 조회 결과:', JSON.stringify(accountResult, null, 2));
        ...
        console.error('일반회원 대시보드 - 이메일로 조회 오류:', queryError);
        throw queryError;
      }
      ...
      try {
        ...
        console.log('일반회원 대시보드 - 이름으로 계정 조회:', userName);
        ...
        console.log('일반회원 대시보드 - 이름으로 개인정보 조회 결과:', JSON.stringify(personResult, null, 2));
        ...
        if (ins_seq !== -1) {
          return NextResponse.json({ 
            error: '일반 회원 계정이 아닙니다.', 
            session: session.user 
          }, { status: 403 });
        }
        
      } catch (queryError) {
        console.error('일반회원 대시보드 - 이름으로 조회 오류:', queryError);
        throw queryError;
      }
    } else {
      console.log('일반회원 대시보드 - 사용자 식별 정보 없음');
      return NextResponse.json({ 
        error: '사용자 정보가 없습니다.', 
        session: session.user 
      }, { status: 400 });
