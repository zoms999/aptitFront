...
export async function GET() {
  try {
    ...
    if (!session || !session.user) {
      console.log('기관회원 대시보드 - 인증되지 않은 세션');
    ...
    if (userAny.id && userAny.ac_id) {
      ac_gid = userAny.id;
      console.log('기관회원 대시보드 - ac_gid 확인:', ac_gid);
      ...
      try {
        ...
        console.log('기관회원 대시보드 - 계정 검증 결과:', JSON.stringify(validateResult, null, 2));
        ...
        console.error('기관회원 대시보드 - 계정 검증 오류:', queryError);
        throw queryError;
      }
    } else {
      console.log('기관회원 대시보드 - 세션 정보 불완전');
    ...
    try {
      ...
      console.log('기관회원 대시보드 - 기관 정보 조회 결과:', JSON.stringify(instituteResult, null, 2));
      ...
      console.error('기관회원 대시보드 - 데이터 조회 오류:', queryError);
      throw queryError;
    }
  } catch (error) {
